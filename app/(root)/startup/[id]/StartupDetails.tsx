...
const page = async ({ id }: StartupDetailsProps) => {
  ...
  const editorPost = playlist?.select.filter((p: {isDeleted: boolean}) => !p.isDeleted) ?? [];
  ...
  return (
    <>
      <section className="pink_container flex flex-col justify-center m-10 !min-h-[230px]">
        <p className="tag text-center p-4 font-bold text-2xl">
          {formatDate(post?._createdAt)}
        </p>
        <h1 className="heading text-white p-8 bg-black text-center text-3xl font-extrabold rounded-xl mb-10">
          {post.title}
        </h1>
